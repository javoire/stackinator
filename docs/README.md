...
A minimal CLI tool for managing stacked branches and syncing them to GitHub Pull Requests.

Inspired by tools like [Charcoal](https://github.com/danerwilliams/charcoal) and [Graphite](https://graphite.dev/).
...
- 🎯 **Minimal State** — Uses git config to track parent relationships, no extra files
...
| Command                   | Description                                  |
| ------------------------- | -------------------------------------------- |
| `stack new <name>`        | Create a new branch in the stack             |
...
| `stack rename <name>`     | Rename branch preserving stack relationships |
| `stack reparent <parent>` | Change the parent of the current branch      |
| `stack worktree <name>`   | Create a worktree for a branch               |

See [Commands Reference](commands.md) for full documentation.
